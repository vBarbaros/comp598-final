...
# OUT_FILE_JSON = 'data/twitter_dump_replies_cbc_only_filter.json'
# OUT_FILE_CSV = 'data/twitter_dump_replies_cbc_only_filter.csv'
# raw_sample_file = "../data/twitter_dump_replies_cbc_only.csv"
...
def collect_valid_tweets(df_small_reindex):
    ...
    for index, row in df_small_reindex.iterrows():
        if check_word_in_text(row['text']) > 10:
            if index < (len(df_small_reindex) - 1) and similar(row['text'],
                                                               df_small_reindex.iloc[index + 1]['text']) < 0.3:
...
def main():
    ...
    df_final = collect_valid_tweets(df_small_reindex)
